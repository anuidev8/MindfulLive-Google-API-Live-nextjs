...
// FIXED: Tool Declarations Following Google Standards
...
const mockProgressSessions = [
  { date: "2024-06-01", duration: 10, outcome: "relaxed" },
  { date: "2024-06-02", duration: 15, outcome: "focused" },
  { date: "2024-06-03", duration: 5, outcome: "stressed" },
];
const mockHealthState = "stressed";

function getRecommendedDuration(progress: { duration: number }[], health: string): number {
  if (health === "stressed") return 15;
  if (health === "relaxed") return 5;
  const durations = progress.map((s) => s.duration);
  const freq: Record<number, number> = {};
  durations.forEach((d) => { freq[d] = (freq[d] || 0) + 1; });
  return +Object.entries(freq).sort((a, b) => (b[1] as number) - (a[1] as number))[0][0];
}
